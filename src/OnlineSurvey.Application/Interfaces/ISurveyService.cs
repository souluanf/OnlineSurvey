...
{
    Task<SurveyDetailResponse> CreateSurveyAsync(CreateSurveyRequest request, CancellationToken cancellationToken = default);
    Task<SurveyDetailResponse?> GetSurveyByIdAsync(Guid id, CancellationToken cancellationToken = default);
    Task<PaginatedResponse<SurveyResponse>> GetSurveysAsync(int page, int pageSize, SurveyStatus? status = null, CancellationToken cancellationToken = default);
